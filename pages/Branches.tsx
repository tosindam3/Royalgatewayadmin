...
const Branches: React.FC = () => {
  ...
  const { data: branchesResponse, isLoading: isListLoading } = useQuery({
    queryKey: ['branches'],
      ...
      <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20">
        <div className="flex justify-between items-center px-4">
          <div>
            <h2 className="text-3xl font-black text-white tracking-tight uppercase italic">
              {formBranch.id ? 'Configure' : 'Provision'} <span className="text-[#8252e9]">Branch</span>
            </h2>
        ...
        <GlassCard className="!p-8 border-t-4 border-t-[#8252e9]">
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-sm text-white font-mono uppercase focus:border-[#8252e9] outline-none" 
                      placeholder="LON-01" 
                  ...
                  <div>
                    ...
                    <select 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-xs text-white focus:border-[#8252e9] outline-none appearance-none cursor-pointer"
                    >
                      {TIMEZONES.map(tz => <option key={tz} value={tz} className="bg-[#0d0a1a]">{tz}</option>)}
                    </select>
                  </div>
               </div>
               <div>
                  ...
                  <textarea 
                    ...
                    rows={3} 
                    className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-3.5 text-sm text-white focus:border-[#8252e9] outline-none transition-all resize-none" 
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-4 py-3.5 text-sm text-white focus:border-[#8252e9] outline-none" 
                      placeholder="City" 
                    />
                  </div>
                  <div>
            ...
            <div className="space-y-6">
               ...
               <div className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-3xl space-y-4">
                  <div className="flex items-center justify-between">
                     <div>
                        <p className="text-xs font-black text-white uppercase italic">Designate Corporate HQ</p>
                        <p className="text-[9px] text-slate-500 font-bold uppercase">Unlocks global control parameters</p>
                     </div>
                     <button 
          ...
          <div className="flex justify-end gap-4 mt-12 pt-8 border-t border-white/5">
             <button onClick={() => setView('LIST')} className="px-8 py-3.5 text-slate-500 hover:text-white font-black text-[11px] uppercase tracking-widest transition-all">Discard</button>
             <button 
             ...
             >
                {createMutation.isPending || updateMutation.isPending ? 'Saving...' : 'Deploy Configuration'}
             </button>
          </div>
        </GlassCard>
      </div>
      ...
      <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500 pb-20">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1 space-y-6">
            <GlassCard title="Node Metadata" className="!p-5 border-l-4 border-l-[#8252e9]">
               <div className="space-y-4">
                 ...
                 <div className="pt-4 border-t border-white/5 space-y-3">
                   <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1 leading-relaxed">{selectedBranch.address || selectedBranch.location}</p>
                   </div>
                   <div>
            ...
            <GlassCard title="Workforce" className="!p-5">
                   ...
                   <GlassCard title="Branch Information">
                      <div className="space-y-4">
                         <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                            ...
                            <span className="text-sm font-black text-white">{selectedBranch.type}</span>
                         </div>
                         <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                         ...
                         <div className="flex justify-between items-center p-4 bg-white/5 rounded-2xl border border-white/5">
                            <span className="text-xs text-slate-400 uppercase">Employees</span>
                            <span className="text-sm font-black text-white">{selectedBranch.employee_count}</span>
                         </div>
                      </div>
                   </GlassCard>
                 </div>
                          ...
                          <tbody className="divide-y divide-white/5">
                             {employees.map((emp: any) => (
                               <tr key={emp.id} className="hover:bg-white/[0.02] transition-colors group">
                                 ...
                                 <td className="px-6 py-4 text-xs text-slate-400 font-medium">{emp.designation?.name || 'N/A'}</td>
                                 <td className="px-6 py-4">
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/10 pb-6">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tight uppercase italic">Distributed <span className="text-[#8252e9]">Infrastructure</span></h2>
                     ...
                     <div className="grid grid-cols-2 gap-2">
                       {['All', 'HQ', 'Regional', 'Satellite'].map(t => (
                         <button 
                            ...
                            className={`py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${filterType === t ? 'bg-white/10 border-[#8252e9] text-white' : 'bg-white/5 border-transparent text-slate-500 hover:text-slate-300'}`}
                         >
                            {t}
                         </button>
                       ))}
                     ...
                     <div className="grid grid-cols-2 gap-2">
                       {['All', 'Active', 'Inactive'].map(s => (
                         <button 
                            ...
                            className={`py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${filterStatus === s ? 'bg-white/10 border-[#8252e9] text-white' : 'bg-white/5 border-transparent text-slate-500 hover:text-slate-300'}`}
                         >
                            {s}
                         </button>
                       ))}
                        ...
                        {filteredBranches.map((branch: Branch) => (
                           <tr key={branch.id} className="group hover:bg-white/[0.03] transition-all cursor-pointer" onClick={() => handleViewDetail(branch.id)}>
                              <td className="px-6 py-6">
                                 <div className="flex items-center gap-4">
                                    <div className={`w-11 h-11 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-xl transition-all group-hover:border-[#8252e9]/50 ${branch.is_hq ? 'shadow-[0_0_15px_rgba(130,82,233,0.3)]' : ''}`}>
                                       {branch.type === 'HQ' ? '🏢' : '🏗️'}
                                    </div>
                                    <div>
